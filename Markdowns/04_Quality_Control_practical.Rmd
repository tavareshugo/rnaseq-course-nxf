...
long for the practical. 
You should find a file called `SRR7657883.salmon.sorted.bam` under the directory `salmon_qc_demo/SRR7657883`,
please use this file rather than the bam file you created in the previous session.
...
What is the duplication rate for this bam file? You'll need to look at the
metrics file. The easiest way is to open in a spreadsheet. On the course 
machines we have LibreOffice Calc. You can find this in the launcher bar:

![](images/Launcher_bar_cacl.png)

<details><summary>Answer</summary>

We ran the command shown, which includes: 

- `salmon_qc_demo/SRR7657883/SRR7657883.salmon.mkdup.bam` - the BAM file with marked duplicates.
- `salmon_qc_demo/SRR7657883/SRR7657883.salmon.mkdup.bai` - the index for the BAM file.
- `salmon_qc_demo/SRR7657883/SRR7657883.salmon.mkdup_metrics.txt` - the duplication metrics will be writen to this file.

Opening that last file in LibreOffice shows the following:

![](images/MarkDuplicates_results.png)

The duplication rate reported is ~5%. 

Note that although the column headers for Picard say "PERCENT" or "PCT" the
number is in fact the decimal fraction and needs to be multiplied by 100 for
...
1. Run Picard's `CollectRnaSeqMetrics` tool on the sorted bam file using the following options:
   * `INPUT` - The bam file
...
2. Open the html report that was generated by multiqc and inspect the QC plots
   One way to do this from the command line is to type 
...
2. Open the html report that was generated by multiqc and inspect the QC plots.
   Are there any samples that look problematic?
