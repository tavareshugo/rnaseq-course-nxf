...
```bash
zcat references/Mus_musculus.GRCm38.dna_sm.primary_assembly.fa.gz | grep ">" | sed 's/>//' | sed 's/ .*//'
```
...
* `--writeMappings=results/salmon/SRR7657872.salmon.sam` - Instructs Salmon to output the read alignments in SAM format to the file `salmon_output/SRR7657883.salmon.sam`.
